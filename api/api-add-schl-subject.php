...
<?php
    ...
    $sql = "SELECT subjectTaught FROM schoolteachers WHERE teacherId=$tid ";
    ...
    if(mysqli_num_rows($result)>0 && strlen($jsonString)>0){
      ...
      if($output[0]['subjectTaught']!="" && $output[0]['subjectTaught']!='<p style="color:red">Pending - Yet to be filled by teacher </p>' && $output[0]['subjectTaught']!='[]'){
        ...
        $len = count($test);
        for($i = 0 ; $i < $len ; $i = $i + 1){
          $semester = strtolower($cameToAdd['std']);
          $subject = strtolower($cameToAdd['sub']);
  
          $semesterTest = strtolower($test[$i]['std']);
          $subjectTest = strtolower($test[$i]['sub']);
  
  
          if($semester==$semesterTest && $subject==$subjectTest){
            $duplicate = true;
            break;
          }
        ...
        if($duplicate==true){
          ...
          $sql2 = "UPDATE schoolteachers SET subjectTaught = '$mergedString'  WHERE teacherId=$tid ";
        ...
        $sql2 = "UPDATE schoolteachers SET subjectTaught = '$jsonString' WHERE teacherId=$tid ";
        $result2 = mysqli_query($conn,$sql2) or die("SQL Query Failed");
