...
<?php
    ...
    if($role == "teacher"){
        ...
        if($clgId){
          ...
          $sql = "SELECT *,(SELECT COUNT(*) FROM users JOIN college ON users.college_id = college.collegeId JOIN collegestudents ON users.userId=collegestudents.user_id WHERE role='student' AND users.college_id='$clgId' AND branch='$branch') AS len FROM users JOIN college ON users.college_id = college.collegeId JOIN collegestudents ON users.userId=collegestudents.user_id WHERE role='$role' AND users.college_id='$clgId' AND branch='$branch' ORDER BY userId DESC LIMIT $start,$limit";
        } else{
