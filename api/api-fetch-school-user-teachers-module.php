<?php
    ...
    if($role == "teacher"){
      ...
      if($schlId){
        $sql = "SELECT *,(SELECT COUNT(*) FROM users JOIN schoolname ON users.schoolname_id = schoolname.schoolnameId JOIN schoolstudents ON users.userId=schoolstudents.user_id WHERE role='student' AND schoolname_id='$schlId' AND standard='$standard') AS len FROM users JOIN schoolname ON users.schoolname_id = schoolname.schoolnameId JOIN schoolstudents ON users.userId=schoolstudents.user_id WHERE role='$role' AND schoolname_id='$schlId' AND standard='$standard' ORDER BY userId DESC LIMIT $start,$limit";
