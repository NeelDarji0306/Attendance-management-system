...
<?php
    ...
    // $sql = "";
    if($role=="teacher"){
        $sql = "SELECT *,(SELECT COUNT(*) FROM users JOIN college ON users.college_id = college.collegeId JOIN collegeteachers ON users.userId=collegeteachers.user_id WHERE role='$role' AND (CONCAT(firstName,' ',lastName) LIKE '%{$search}%' OR email LIKE '%{$search}%' OR collegeName LIKE '%{$search}%')) AS len FROM users JOIN college ON users.college_id = college.collegeId JOIN collegeteachers ON users.userId=collegeteachers.user_id WHERE role='$role' AND (CONCAT(firstName,' ',lastName) LIKE '%{$search}%' OR email LIKE '%{$search}%' OR collegeName LIKE '%{$search}%') ORDER BY userId DESC LIMIT $start,$limit";
