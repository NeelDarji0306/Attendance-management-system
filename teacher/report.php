...
if(isset($_GET["action"]))
{
	...
	if($_GET["action"] == "clg_attendance_report")
	{
		...
		if(isset($_GET["from_date"], $_GET["to_date"]))
		{
			...
			WHERE teacher_id = '$tid' AND department='$dep' AND sem='$sem' AND subject='$sub' AND laborlec='$laborlec' AND college_id='$clgid'
	...
	if($_GET["action"] == "schl_attendance_report")
	{
		...
		if(isset($_GET["from_date"], $_GET["to_date"]))
		{
			...
			WHERE teacher_id = '$tid' AND standard='$std' AND subject='$sub' AND schoolname_id='$schlid'
			AND (date BETWEEN '$from_date' AND '$to_date')
			ORDER BY date ASC
			";
